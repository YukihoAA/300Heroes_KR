...
-------装备界面背包扩展
function InitBag_ExpendUI(wnd, bisopen)
	----------背包扩展
	g_bag_expend_ui = CreateWindow(wnd.id, 700,220,410,460)
	for i=1,18 do
		local posx = 54*((i-1)%7+1)+10
		local posy = math.ceil(i/7)*54-44
		
		bag_wnd[i] = g_bag_expend_ui:AddImage(path_equip.."bag_equip.png",posx,posy,50,50)
			...
			if(g_BagStorage_ui:IsVisible() == true) then
			    ...
			    if(mndex == 0) then
				   return
                    ...
                    XEquip_BagStorage_change(4,2,i,mndex)
                end	
	...
	g_bag_expend_ui:AddImage(path_equip.."blacklight_equip.png",150,220,202,10)
	StrExp = g_bag_expend_ui:AddImage(path_equip.."pinklight_equip.png",150,220,202,10)
	StrExp:SetAddImageRect(StrExp.id, 0, 0, 202*(CurExp/MaxExp), 10, 150 ,220, 202*(CurExp/MaxExp), 10)
	
	local AM = g_bag_expend_ui:AddFont("提示：\n◆拖动物品移至最上方的背包标签，再松开即可完成物品的转移.",11,0,80,330,340,40,0x8295cf)
	AM:SetFontSpace(1,1)
	BagActive = g_bag_expend_ui:AddButton(path_setup.."buy1_setup.png",path_setup.."buy2_setup.png",path_setup.."buy3_setup.png",210,250,83,35)
	...
	destory = g_bag_expend_ui:AddImage(path_equip.."destory_equip.png",65,400,32,32)
	XWindowEnableAlphaTouch(destory.id)
	g_bag_expend_ui:AddFont("物品销毁",11, 0, 85, 404, 100, 20, 0x8295cf)

	sort_btn = g_bag_expend_ui:AddButton(path_setup.."btn1_mail.png",path_setup.."btn2_mail.png",path_setup.."btn3_mail.png", 245, 395, 100, 32)
	...
	repair_btn = g_bag_expend_ui:AddButton(path_setup.."btn1_mail.png",path_setup.."btn2_mail.png",path_setup.."btn3_mail.png", 345, 395, 100, 32)
	repair_btn:AddFont("全部修理",12, 8, 0, 0, 100, 32, 0xffffff)
	repair_btn.script[XE_LBUP] = function()
	    XEquip_BagEquipRpaireAll()--需要区分
	end
	g_bag_expend_ui:SetVisible(bisopen)
end

function GetBagExpand_empty()
    for index = 1,18 do
...
function SetBagExpandlockVisible(index)
   for i = 1,18 do
      bag_wnd.lock[i]:SetVisible(index)
   end
end

function SetBagExpandStation(ibool)
	if(ibool == 1) then
		BagActivefont:SetFontText("续费背包")
        SetBagExpandlockVisible(0)		
	elseif(ibool == 0) then
		BagActivefont:SetFontText("激活背包")
		BagTimeLimit:SetFontText("有效期已过只能取出不能存入",0x6ffefc)
		SetBagExpandlockVisible(1)
	end
end

function SetBagExpandTimeLimit(Ctext,iday)
...
function SetBagTimeLimit(index,Ctext)
    if(index == 2) then
	    Bag2.TimeLimit:SetFontText(Ctext,0x6ffefc)
	elseif(index == 3) then
	    Bag3.TimeLimit:SetFontText(Ctext,0x6ffefc)
	end
end



function Equip_BagExpand_cleanALl_ItemInfo()
    for index =1,18 do
...
function Equip_BagExpand_needEquipInfo()--需要全部 导入第一个背包数据
    if(g_bag_expend_ui:IsVisible() == false) then
        return
	end	
	Equip_BagExpand_cleanALl_ItemInfo()--equipManage.CpackCindex = {}
    for index,value in pairs(equipManage.CitemIndex) do
	    local packCindex = equipManage.CpackCindex[index]+1
	    if(equipManage.bagType[index] == 4 and equipManage.CpackCindex[index] <=18) then
...
---用于拖拽---
function Equip_BagExpand_pullPicXLUP()
    if(g_bag_expend_ui:IsVisible() == false or pullPicType.id == 0 or pullPicType.id ==nil) then
	    return
	end
    local tempRanking = Equip_BagExpand_checkRect() --判断是否在包裹框
	local oldBag = equipManage.bagType[pullPicType.id]
	local oldranking = equipManage.CpackCindex[pullPicType.id]+1  --得到老的位置
	
	if(oldranking<=18 and oldranking>=1 and oldBag == 4) then
...
function Equip_BagExpand_checkRect()
    for i = 1,18 do
...
function Equip_BagExpand_getPullPicIndexInPackage(id) --得到老的位置
   for i = 1,18 do
...
function Equip_BagExpand_PullPicDeleteXLUP()  
    if(g_bag_expend_ui:IsVisible() == false) then
	    return
	end
...
function SetBagExpendIsVisible(flag) 
	if g_bag_expend_ui ~= nil then
		if flag == 1 and g_bag_expend_ui:IsVisible() == false then
			g_bag_expend_ui:SetVisible(1)
			EquipChangeBag(4)
			Equip_BagExpand_needEquipInfo()
		elseif flag == 0 and g_bag_expend_ui:IsVisible() == true then
			g_bag_expend_ui:SetVisible(0)
			Equip_BagExpand_cleanALl_ItemInfo()
		end
	end
end

function Bag_BagExpendSetPosition(x,y)
	g_bag_expend_ui:SetPosition(x,y)
end