...
-- 界面控件
local StoneSearchInputEdit = nil
local StoneSearchInput = nil
-- local StoneSearchInputText = nil
...
function IsShop_Sec_StoneInputFocus()
	if StoneSearchInput ~= nil then
		return StoneSearchInput:IsFocus()
	end
	return false
end

function InitShop_Sec_StoneUI(wnd, bisopen)
	g_shop_Sec_Stone_ui = CreateWindow(wnd.id, 50, 150, 900, 600)
	InitMainShop_Sec_Stone(g_shop_Sec_Stone_ui)
	g_shop_Sec_Stone_ui:SetVisible(bisopen)
end
function InitMainShop_Sec_Stone(wnd)

	-- 宝石搜索
	StoneSearchInputEdit = CreateWindow(wnd.id, 950+ppx,165+ppy, 256, 32)
	StoneSearchInput = StoneSearchInputEdit:AddEdit(path_shop.."InputEdit_shop.png","","onSec_StoneSearch_Enter","",13,5,5,230,25,0xffffffff,0xff000000,0,"")
	XEditSetMaxByteLength(StoneSearchInput.id,60)
	StoneSearchInput:SetDefaultFontText("搜索按Enter键确定", 0xff303b4a)
	...
	-- ADHave:AddFont("攻击类", 15, 0, 28, 5, 100, 20, 0xc7bcf6)
	...
	-- APHave:AddFont("法术类", 15, 0, 28, 5, 100, 20, 0xc7bcf6)
	...
	-- TANKHave:AddFont("防御类", 15, 0, 28, 5, 100, 20, 0xc7bcf6)
	...
	-- CSHave:AddFont("冷却/速度", 15, 0, 18, 5, 100, 20, 0xc7bcf6)
	...
	for i=1, 12 do
		...
		g_item_ui[i] = g_shop_Sec_Stone_ui:AddImage(path_shop.."ITEMBK_shop.png",g_item_posx[i],g_item_posy[i],212,195)
...
-- 宝石搜索
function IsFocusOn_StoneSearch()
	if (g_shop_Sec_Stone_ui:IsVisible() == true) then
		-- 搜索框框
		local Input_Focus = StoneSearchInput:IsFocus()
		
		if Input_Focus == true then
		
		elseif Input_Focus == false then
			-- StoneSearchInput:SetEdit("")
		end
	end
end
...
function SetShop_Sec_StoneIsVisible(flag) 
	if g_shop_Sec_Stone_ui ~= nil then
...
function GetShop_Sec_StoneIsVisible()  
    if(g_shop_Sec_Stone_ui:IsVisible()) then
...
function ResetSecStoneScrollList()
	-- 让控件和滚动条保持一致
	Many = 0
	Many_Equip = Many
	updownCount = 0
...
-- 根据数据创建Item
function CreateItemFrame_SecStone()
	return SecStone_RedrawUI()
end

-- 得到物品个数
function GetItemCount_SecStone(ItemCount)
	AllCount = ItemCount
	if AllCount < 13 then
	...
	return CreateItemFrame_SecStone()
end

-- 绘制商品信息界面
function SecStone_RedrawUI()
	for i=1, #g_item_ui do
		if ((i+Many*4) > AllCount) then
			...
			if (EquipInfo.strPictureAdr[i+Many*4] == "" or EquipInfo.strPictureAdr[i+Many*4] == nil) then
				-- 如果图片路劲为空则返回
				log("\nErrorIconPathFrom [shop_Sec_stone]")
			else
...
-- 接收C++传来的商品信息
function SecStone_ReceiveEquipInfo(strName,strPictureAdr,strDesc,leftTime,money,gold,honour,contri,vip,m_Flag,IsCanBuy,Id,ItemId, ctip)
...
-- 清空商品信息表
function SecStone_clearEquipInfo()
	-- ResetSecStoneScrollList()
...
function GetFindInputChar_stone()
	if g_shop_Sec_Stone_ui:IsVisible()==true then
		XTellCInputChar(StoneSearchInput.id)
	end
end

function ReSetItemPosAndVisible_SecStone()
	SecStone_RedrawUI()
end