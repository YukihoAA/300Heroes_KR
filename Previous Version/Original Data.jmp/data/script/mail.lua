...
function InitMain_Mail(wnd)
	...
	-- 勾选框的创建
	img_MailBg:AddFont("全选", 15, 0, 52, 102, 50, 20, 0xff634792)
	...
	local AA = img_MailBg:AddFont("提示：邮件会在收到7天后删除，有附件未领的邮件则延长至30天", 11, 0, 15, 460, 400, 12, 0xb27936)
	AA:SetFontSpace(1,1)
	-- 邮件列表创建
	local iX,iY = 12,135
	for i = 1, 8 do
		...
		MailItemList.Name[i] = MailItemList[i]:AddFont("日常：每日登陆"..i, 12, 8, -107, -10, 125, 20,  0xff87ced4)
		MailItemList.Date[i] = MailItemList[i]:AddFont("2015.8.1"..i, 12, 0, 242, 10, 100, 20,  0xff87ced4)
		MailItemList.LastDay[i] = MailItemList[i]:AddFont(i.."1天", 12, 0, 317, 10, 60, 20,  0xff87ced4)
	...
	local Fa = btn_TackAll:AddFont("全部提取", 15, 8, 0, 0, 100, 32, 0xffffff)
	...
	local Fb = btn_DelMail:AddFont("删除全部", 15, 8, 0, 0, 100, 32, 0xffffff)
	Fb:SetTouchEnabled(0)
	local AM = btn_DelMail:AddFont("含附件的邮件无法删除",11, 0, -12, 31, 150, 12, 0xb27936)
	AM:SetFontSpace(1,1)
	...
	font_MailInfoName = img_MailInfoBg:AddFont("日常：每日登陆", 15, 0, 69, 70, 300, 20, 0xff87ced4)
	img_MailInfoBg1 = img_MailInfoBg:AddImage(path_setup.."InfoBg_mail.png", 27, 99, 458, 228)
	font_MailInfo = img_MailInfoBg1:AddFont("角色名称七个字", 15, 0, 10, 10, 440, 200, 0xffffffff)
	font_MailInfoLastDay = img_MailInfoBg1:AddFont("邮件保存期还有：XX天", 11, 0, 330, 205, 120, 11, 0xc23452)
	...
	local Fa = btn_DelMailInfo:AddFont("删除邮件", 15, 8, 0, 0, 100, 32, 0xffffff)
	Fa:SetTouchEnabled(0)
	...
	local Fb = btn_TackMailInfo:AddFont("提取附件", 15, 8, 0, 0, 100, 32, 0xffffff)
	Fb:SetTouchEnabled(0)
...
function ReFresh()
	...
	for i = 1, iCount do
		...
		MailItemList.LastDay[i]:SetFontText(MailInfo.LastDay[i].."天", 0xff87ced4)
